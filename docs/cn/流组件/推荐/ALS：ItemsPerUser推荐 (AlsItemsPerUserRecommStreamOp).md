# ALS：ItemsPerUser推荐 (AlsItemsPerUserRecommStreamOp)
Java 类名：com.alibaba.alink.operator.stream.recommendation.AlsItemsPerUserRecommStreamOp

Python 类名：AlsItemsPerUserRecommStreamOp


## 功能介绍
使用ALS (Alternating Lease Square）训练的模型为user 实时推荐items。这里的ALS模型可以是隐式模型，也可以是显式模型，输出格式是MTable。
...
public class AlsItemsPerUserRecommStreamOpTest {
	@Test
	public void testAlsItemsPerUserRecommStreamOp() throws Exception {
		...
		StreamOperator <?> predictor = new AlsItemsPerUserRecommStreamOp(model)
			.setUserCol("user").setRecommCol("rec").setK(1).setReservedCols("user");
		predictor.linkFrom(sdata).print();
		StreamOperator.execute();
	}
