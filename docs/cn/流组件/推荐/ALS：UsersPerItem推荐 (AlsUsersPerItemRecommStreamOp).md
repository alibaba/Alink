# ALS：UsersPerItem推荐 (AlsUsersPerItemRecommStreamOp)
Java 类名：com.alibaba.alink.operator.stream.recommendation.AlsUsersPerItemRecommStreamOp

Python 类名：AlsUsersPerItemRecommStreamOp


## 功能介绍
使用ALS (Alternating Lease Square）训练的模型为item 实时推荐users。这里的ALS模型可以是隐式模型，也可以是显式模型，输出格式是MTable。
...
predictor = AlsUsersPerItemRecommStreamOp(model) \
...
public class AlsUsersPerItemRecommStreamOpTest {
	@Test
	public void testAlsUsersPerItemRecommStreamOp() throws Exception {
		...
		StreamOperator <?> predictor = new AlsUsersPerItemRecommStreamOp(model)
			.setItemCol("item").setRecommCol("rec").setK(1).setReservedCols("item");
		predictor.linkFrom(sdata).print();
		StreamOperator.execute();
	}
