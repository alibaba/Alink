# UserCf：打分推荐 (UserCfRateRecommStreamOp)
...
predictor = UserCfRateRecommStreamOp(model)\
    .setUserCol("user")\
    .setItemCol("item")\
...
import com.alibaba.alink.operator.stream.recommendation.UserCfRateRecommStreamOp;
...
public class UserCfRateRecommStreamOpTest {
	@Test
	public void testUserCfRateRecommStreamOp() throws Exception {
		...
		BatchOperator <?> model = new UserCfTrainBatchOp()
			...
			.setItemCol("item")
			.setRateCol("rating").linkFrom(data);
		StreamOperator <?> predictor = new UserCfRateRecommStreamOp(model)
			.setUserCol("user")
			.setItemCol("item")
			.setRecommCol("prediction_result");
		predictor.linkFrom(sdata).print();
