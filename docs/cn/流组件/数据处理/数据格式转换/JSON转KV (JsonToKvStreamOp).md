# JSON转KV (JsonToKvStreamOp)
Java 类名：com.alibaba.alink.operator.stream.dataproc.format.JsonToKvStreamOp

Python 类名：JsonToKvStreamOp
...
import com.alibaba.alink.operator.stream.dataproc.format.JsonToKvStreamOp;
...
public class JsonToKvStreamOpTest {
	@Test
	public void testJsonToKvStreamOp() throws Exception {
		List <Row> df = Arrays.asList(
			Row.of("1", "{\"f0\":\"1.0\",\"f1\":\"2.0\"}", "$3$0:1.0 1:2.0", "f0:1.0,f1:2.0", "1.0,2.0", 1.0, 2.0)
		);
		StreamOperator <?> data = new MemSourceStreamOp(df,
			"row string, json string, vec string, kv string, csv string, f0 double, f1 double");
		StreamOperator <?> op = new JsonToKvStreamOp()
			.setJsonCol("json")
			.setReservedCols("row")
			.setKvCol("kv")
			.linkFrom(data);
