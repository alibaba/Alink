# Bayes搜索TV (BayesSearchTVSplit)
Java 类名：com.alibaba.alink.pipeline.tuning.BayesSearchTVSplit

Python 类名：BayesSearchTVSplit


## 功能介绍

BayesSearchTVSplit是通过构造参数的分布，对其中的每一组输入参数的组很分别进行训练，预测，评估。取得评估参数最优的模型，作为最终的返回模型
...
def rf_grid_search_tv(featureCols, categoryFeatureCols, label, metric):
    ...
    tuningEvaluator = (
        ...
        .setLabelCol(label)
        .setPredictionDetailCol("prediction_detail")
        .setTuningBinaryClassMetricå(metric)
    )
    cv = (
        GridSearchTVSplit()
        .setEstimator(rf)
