...
class TestPinyi(unittest.TestCase):
    ...
    def test_max_abs_scaler_op(self):
        data = np.array([
            ["a", 10.0, 100],
            ["b", -2.5, 9],
            ["c", 100.2, 1],
        ...
        from pyalink.alink.common.types import MaxAbsScalarModelInfo

        def model_info_callback(d: MaxAbsScalarModelInfo):
            self.assertEquals(type(d), MaxAbsScalarModelInfo)
            print("1:", d.getMaxAbs())
