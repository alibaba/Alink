...
public class ReadAudioToTensorMapper extends Mapper {
	...
	public static final class AudioToFloatTensor {
		public static Tuple2<Long, FloatTensor> read(
			...
			duration = duration == -1 ? frameLen : duration * sampleRate + offset;
			duration = duration > frameLen ? frameLen : duration;
			int sampleStart = (int) offset;
			int sampleEnd = (int) duration;
			// 拼读音频byte，转float
			float floatOffset;
			float floatScale;
			float[][] data = new float[channelNum][sampleEnd - sampleStart];
			if (sampleSize > 8) {
			...
			if (frameRate != sampleRate) {
				...
				float[][] resampleDate = new float[channelNum][newWindow * winNum];
