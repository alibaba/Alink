...
public class SpearmanCorrelation {
	 ...
	 * @return rank by col.
	 */
	static DataSet <Row> calcRank(DataSet <Row> data, Boolean outputIsVector, boolean hasRowId) {
		...
		//TripleComparable: sort by colId and value
		...
		return sortedData.f0
			...
			.reduceGroup(new CombineRank(outputIsVector, hasRowId));
	}
	...
	public static class CombineRank extends RichGroupReduceFunction <Tuple3 <Integer, Long, Long>, Row> {
		...
		CombineRank(boolean outputIsVector, boolean hasRowId) {
		...
		public void reduce(Iterable <Tuple3 <Integer, Long, Long>> iterable, Collector <Row> collector)
			...
			if (hasRowId) {
				int n = out.getArity();
				Row outAppendId = new Row(n + 1);
				for (int i = 0; i < n; i++) {
					outAppendId.setField(i, out.getField(i));
				}
				outAppendId.setField(n, rowId);
				out = outAppendId;
			}
