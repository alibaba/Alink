...
public class DeepARPreProcessBatchOp extends BatchOperator <DeepARPreProcessBatchOp>
	...
	public DeepARPreProcessBatchOp linkFrom(BatchOperator <?>... inputs) {
		...
		if (inputs.length > 1) {
			frequency = inputs[1]
				...
				.map(new MapFunction <Row, TimeFrequency>() {
					...
					public TimeFrequency map(Row value) throws Exception {
						return Params.fromJson(String.valueOf(value.getField(0))).get(HasTimeFrequency.TIME_FREQUENCY);
					}
				});
		} else {
			...
			frequency = input
				...
				.mapPartition(new MapPartitionFunction <Row, Tuple2 <Timestamp, Timestamp>>() {
					@Override
					public void mapPartition(Iterable <Row> values, Collector <Tuple2 <Timestamp, Timestamp>> out) {
		...
		TypeInformation <?>[] outputColTypes = genY ?
			new TypeInformation <?>[] {TensorTypes.FLOAT_TENSOR, TensorTypes.FLOAT_TENSOR, TensorTypes.FLOAT_TENSOR} :
				...
				.mapPartition(
					new RichMapPartitionFunction <Tuple2 <Integer, Row>, Row>() {
						...
						public void mapPartition(Iterable <Tuple2 <Integer, Row>> values,
							...
							// series id
							final ArrayList <Tuple3 <Integer, FloatTensor, FloatTensor>> tensors = new ArrayList <>();
