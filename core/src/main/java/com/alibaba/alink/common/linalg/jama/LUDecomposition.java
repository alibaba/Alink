...
class LUDecomposition implements java.io.Serializable {
	private static final long serialVersionUID = -4234096950824402122L;
	/* ------------------------
     Class variables
     * ------------------------ */
	 ...
	 */
	public LUDecomposition(JaMatrix A) {
	...
	public JaMatrix solve(JaMatrix B) {
		...
		JaMatrix Xmat = B.getMatrix(piv, 0, nx - 1);
		double[][] X1 = Xmat.getArray();

		// Solve L*Y = B(piv,:)
		for (int k = 0; k < n; k++) {
			for (int i = k + 1; i < n; i++) {
				for (int j = 0; j < nx; j++) {
					X1[i][j] -= X1[k][j] * LU[i][k];
				}
			}
		}
		// Solve U*X = Y;
