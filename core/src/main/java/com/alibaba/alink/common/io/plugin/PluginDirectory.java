...
/**
 * This class is used to create a collection of {@link org.apache.flink.core.plugin.PluginDescriptor} based on directory structure for a given plugin
 * root folder.
 *
 * <p>The expected structure is as follows: the given plugins root folder, containing the flink version based folder.
 * One flink version folder contains all plugins. One plugin folder contains all resources (jar files) belonging to a plugin.
 * The name of the plugin folder becomes the plugin id.
 * <pre>
 * plugins-root-folder/
 *            |------ flink-1.0
...
public class PluginDirectory {
	...
	public PluginDescriptor createPluginDescriptorForSubDirectory(
		...
		if (Files.exists(versionFile) && Files.isRegularFile(versionFile, LinkOption.NOFOLLOW_LINKS)) {
			...
			parentFirst = properties.getProperty(Joiner.on(".").join(PLUGIN_PREFIX, PLUGIN_PARENT_FIREST), parentFirst);
		}
