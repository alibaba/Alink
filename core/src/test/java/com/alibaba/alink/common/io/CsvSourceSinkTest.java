...
public class CsvSourceSinkTest extends AlinkTestBase {
	...
	public void testSkipBlankLine() throws Exception {
		...
		Assert.assertEquals(new CsvSourceBatchOp().setFilePath(filePath).setSchemaStr("f1 string").collect().size(),
			1);
	}
	...
	public void testLenient() throws Exception {
		...
		String filePath = path + "file5.csv";
		Files.write(Paths.get(filePath), data.getBytes(), StandardOpenOption.CREATE);

		//        new CsvSourceBatchOp().setFilePath(filePath).setSchemaStr("f1 string, f2 bigint").setLenient(true)
		// .lazyPrint(-1);
