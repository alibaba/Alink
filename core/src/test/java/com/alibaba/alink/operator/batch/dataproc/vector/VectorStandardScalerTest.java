...
public class VectorStandardScalerTest extends AlinkTestBase {
	...
	public void test() throws Exception {
		...
		VectorStandardScalerTrainBatchOp op = new VectorStandardScalerTrainBatchOp()
			...
			.setSelectedCol("vec")
			.linkFrom(batchData);
		new VectorStandardScalerPredictBatchOp().setOutputCol("vec_1")
			.linkFrom(op, batchData)
			.lazyCollect(new Consumer <List <Row>>() {
				@Override
				public void accept(List <Row> rows) {
					assertDv(VectorUtil.getDenseVector(rows.get(1).getField(1)),
						new DenseVector(new double[] {-0.9272, -1.1547}));
					assertDv(VectorUtil.getDenseVector(rows.get(0).getField(1)),
						new DenseVector(new double[] {-0.1325, 0.5774}));
					assertDv(VectorUtil.getDenseVector(rows.get(2).getField(1)),
