...
public class VectorAssemblerMapperTest extends AlinkTestBase {
	...
	public void testSkip() throws Exception {
		...
		Params params = new Params()
			...
			.set(VectorAssemblerParams.OUTPUT_COL, "out")
			.set(VectorAssemblerParams.HANDLE_INVALID, VectorAssemblerParams.HandleInvalidMethod.SKIP)
			.set(VectorAssemblerParams.RESERVED_COLS, new String[] {"c0"});
	...
	public void testKeep() throws Exception {
		...
		assertEquals(mapper.map(Row.of(new DenseVector(new double[] {3.0, 4.0}), 1,
			new SparseVector(11, new int[] {0, 10}, new double[] {1.0, 4.0}))).getField(1),
			new SparseVector(14, new int[] {0, 1, 2, 3, 13}, new double[] {3.0, 4.0, 1, 1.0, 4.0}));
		assertEquals(mapper.getOutputSchema(), outSchema);
	}
