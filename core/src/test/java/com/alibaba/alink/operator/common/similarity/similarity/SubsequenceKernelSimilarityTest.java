...
public class SubsequenceKernelSimilarityTest extends AlinkTestBase {
	private String s1 = "Good Morning!";
	private String s2 = "Good Evening!";
	private SubsequenceKernelSimilarity similarity = new SubsequenceKernelSimilarity(1, 0.5);

	@Test
	public void calsimilarity() {
		Assert.assertEquals(similarity.calc(s1, s2), 0.84, 0.01);
		Assert.assertEquals(similarity.calc(s1.split(" "), s2.split(" ")), 0.0, 0.01);
	}
	...
	public void calc() {
		...
		Assert.assertEquals(similarity.calc(sample1, sample2, false), 0.84, 0.01);
