...
public class KeywordsExtractionLocalOpTest extends TestCase {
	...
	public void testKeywordsExtractionByTextRankBatch() {
		...
		List <Row> pred = new MemSourceLocalOp(array, new String[] {"ID", "text"})
			.link(
				new KeywordsExtractionLocalOp()
					.setSelectedCol("text")
					.setMethod("TEXT_RANK")
					.setTopN(3)
			)
		...
		assertListRowEqualWithoutOrder(expected, pred);
	}
	...
	public void testKeywordsExtractionByTfIdfBatch() throws Exception {
		...
		new MemSourceLocalOp(array, new String[] {"ID", "text"})
			.link(new KeywordsExtractionLocalOp()
				.setSelectedCol("text")
				.setTopN(9)
				.setMethod("TF_IDF")
			)
