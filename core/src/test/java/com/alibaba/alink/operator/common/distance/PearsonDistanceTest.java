...
public class PearsonDistanceTest extends AlinkTestBase {
	...
	public void testContinuousDistance() {
		Assert.assertEquals(distance.calc(denseVector1, denseVector2), 1.33, 0.01);
		Assert.assertEquals(distance.calc(denseVector1.getData(), denseVector2.getData()), 1.33, 0.01);
		Assert.assertEquals(distance.calc(denseVector1, sparseVector1), 1.27, 0.01);
		Assert.assertEquals(distance.calc(sparseVector1, sparseVector2), 1.5, 0.01);
		Assert.assertEquals(distance.calc(sparseVector1, denseVector2), 1.720, 0.01);
	}
	...
	public void testUpdateLabel() {
		Vector vec = distance.prepareVectorData(Tuple2.of(denseVector1.clone(), null)).getVector();
	...
	public void testCalDistanceVecVec() {
		...
		Assert.assertEquals(denseResult.get(0, 0), 1.33, 0.01);
		...
		Assert.assertEquals(sparseResult.get(0, 0), 1.5, 0.01);
	}
	...
	public void testCalDistanceMatrixMatrix() {
		...
		double[] expect = new double[] {0.0, 1.33, 1.33, 0.0};
	...
	public void testCalDistanceVecMatrix() {
		...
		double[] expect = new double[] {1.27, 1.720};
		for (int i = 0; i < expect.length; i++) {
			Assert.assertEquals(expect[i], predict[i], 0.01);
		}
	...
	public void testCalDistanceVecSparse() {
		...
		double[] expect = new double[] {1.27, 0.18, 0.18};
		for (int i = 0; i < expect.length; i++) {
			Assert.assertEquals(expect[i], predict[i], 0.01);
		}
	...
	public void testCalDistanceSparseSparse() {
		...
		double[] expect = new double[] {0.0, 1.5, 1.5, 1.5, 0.0, 0.0};
		double[] predict = res.getData();
		for (int i = 0; i < expect.length; i++) {
			Assert.assertEquals(expect[i], predict[i], 0.01);
		}
