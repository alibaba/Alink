...
public class StratifiedSampleTest {
    ...
    public void testStratifiedSampleWithRatio() throws Exception {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            StratifiedSampleBatchOp sampleOp = new StratifiedSampleBatchOp("key", "a:11");
            sampleOp.setWithReplacement(true);
    ...
    public void testStratifiedSampleWithRatio1() throws Exception{
        ...
        List<Tuple2<String, Double>> mappingFraction = Arrays.asList(new Tuple2<>("a",0.4),new Tuple2<>("b",0.4),new Tuple2<>("c",0.4));
        for (int i = 0; i < 20; i++) {
            MemSourceBatchOp sourceOp = new MemSourceBatchOp(data, new String[]{"key", "val"});
            StratifiedSampleBatchOp sampleOp = new StratifiedSampleBatchOp("key", Joiner.on(",").join(mappingFraction.
                    stream().
            ...
            double f2 = len[1] * fb;
            double f3 = len[2] * fc;
            Assert.assertTrue("expected number is :" + f1 + " +- " + f1 * 0.1 + " but actual is :" + a,
                    a > f1 * 0.9 && a < f1 * 1.1);
            Assert.assertTrue("expected number is :" + f2 + " +- " + f2 * 0.1 + " but actual is :" + b,
                    b > f2 * 0.9 && b < f2 * 1.1);
            Assert.assertTrue("expected number is :" + f3 + " +- " + f3 * 0.1 + " but actual is :" + c,
                    c > f3 * 0.9 && c < f3 * 1.1);
        }
    }
    ...
    public void testStratifiedSampleWithSize() throws Exception {
        ...
        for (int i = 0; i < 20; i++) {
            MemSourceBatchOp sourceOp = new MemSourceBatchOp(data, new String[]{"key", "val"});
            StratifiedSampleWithSizeBatchOp sampleOp = new StratifiedSampleWithSizeBatchOp("key", "a:100");
            sampleOp.setWithReplacement(true);
    ...
    public void testStratifiedSampleWithSize1() throws Exception{
        ...
        for (int i = 0; i < 20; i++) {
            MemSourceBatchOp sourceOp = new MemSourceBatchOp(data, new String[]{"key", "val"});
            StratifiedSampleWithSizeBatchOp sampleOp = new StratifiedSampleWithSizeBatchOp("key", Joiner.on(",").join(mappingFraction.
                    stream().
                    map(t2 -> Joiner.on(":").join(t2.f0, t2.f1))
                    .collect(Collectors.toList())));
            sampleOp.setWithReplacement(true);
