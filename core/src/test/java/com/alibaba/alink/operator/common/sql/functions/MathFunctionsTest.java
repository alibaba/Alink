...
public class MathFunctionsTest {
	...
	public void testLogWithBase() throws InvocationTargetException, IllegalAccessException {
		...
		Assert.assertEquals(expected,
			(Double) MathFunctions.LOG_WITH_BASE_DEC_DOU.invoke(null, BigDecimal.valueOf(base), v), EPS);
		Assert.assertEquals(expected,
			(Double) MathFunctions.LOG_WITH_BASE_DOU_DEC.invoke(null, base, BigDecimal.valueOf(v)), EPS);
		Assert.assertEquals(expected,
			(Double) MathFunctions.LOG_WITH_BASE_DEC_DEC.invoke(null, BigDecimal.valueOf(base), BigDecimal.valueOf(v)),
			EPS);
	}
