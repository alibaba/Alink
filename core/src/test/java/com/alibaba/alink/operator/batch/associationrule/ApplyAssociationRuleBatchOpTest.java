...
public class ApplyAssociationRuleBatchOpTest extends AlinkTestBase {
	@Test
	public void testFpGrowth() throws Exception {
		...
		ApplyAssociationRuleBatchOp op = new ApplyAssociationRuleBatchOp()
			...
			.setOutputCol("result")
			.linkFrom(fpGrowth.getSideOutputAssociationRules(), BatchOperator.fromTable(data));
		Assert.assertEquals(op.count(), 5);
	}
}
