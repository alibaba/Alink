...
public class KafkaSourceBuilder {
	...
	public RichParallelSourceFunction <Row> build() {
		...
		if (!StringUtils.isNullOrWhitespaceOnly(topicPattern)) {
			...
			consumer = new FlinkKafkaConsumer011 <>(pattern, new MessageDeserialization(), properties);
		} else {
			consumer = new FlinkKafkaConsumer011 <>(topic, new MessageDeserialization(), properties);
		}
